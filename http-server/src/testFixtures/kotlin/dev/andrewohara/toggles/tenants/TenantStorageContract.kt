...
abstract class TenantStorageContract: StorageContractBase() {
    ...
    override fun setup() {
        ...
        tenant1 = Tenant(TenantId.random(random), time).also(storage.tenants::plusAssign)
        tenant2 = Tenant(TenantId.random(random), time).also(storage.tenants::plusAssign)
        tenant3 = Tenant(TenantId.random(random), time).also(storage.tenants::plusAssign)
    }
    ...
    fun `get tenant - not found`() {
        storage.tenants[TenantId.random(random)] shouldBe null
    }
